...
Russia war on Ukraine had an impact to EU and Russia trade. The goal of this document is to give a bird's eye view of these changes using the EU trade data from Eurostat. 
...
4. More that 100 sanctioned export markets has increased by 100% after the war and constitute 21% of total sanctioned exports. This strongly suggests that EU countries are evading sanctions by exporting to Russia neighbours instead.

# EU overall trade with Russia and neighbouring countries 

Energy import constitutes about 70% of total imports. It distorts overall picture so it was excluded from the analysis. Energy exports constitute arounde 1% of total exports, but their calculation is complicated for countries connected to BRELL grid. For this reason it is excluded too.


In after war period EU countries exports to Russia fell by `r round(abs(ru_data$egrowth[1]))`% to `r round(ru_data$export[1]/1e9)` billion euro compared to similar period in 2021. However EU countries export to Russia neighbours rose by `r round(nb_data$egrowth[1])`% to `r round(nb_data$export[1]/1e9)` billion euro. 

In after war period EU countries imports to Russia fell by `r round(abs(ru_data$igrowth[1]))`% to `r round(ru_data$import[1]/1e9)` billion euro compared to similar period in 2021. EU countries import from Russia neighbours rose by `r round(nb_data$igrowth[1])`% to `r round(nb_data$import[1]/1e9)` billion euro. 
...
ggplot(aes(x = reporter, y = egrowth, fill = epos), data = eu_erf3) +
  ...
  labs(y = "Export growth, %", x = "", title = "EU export growth to Russia after war")
```
...
ggplot(aes(x = reporter, y = egrowth, fill = epos), data = eu_enb3) +
  ...
  theme_bw() +
  scale_fill_manual(values = cols_true[c(2, 1)], guide = "none") +
  labs(y = "Export growth, %", x = "", title = "EU export growth to Russia neighbours after war")
```
...
ggplot(aes(x = reporter, y = igrowth, fill = ipos), data = eu_erf5) +
  ...
  theme_bw() +
  scale_fill_manual(values = cols_true[c(2, 1)], guide = "none") +
  labs(y = "Import growth, %", x = "", title = "EU import growth from Russia after war")
```
...
ggplot(aes(x = reporter, y = igrowth, fill = ipos), data = eu_enb5) +
  ...
  theme_bw() +
  scale_fill_manual(values = cols_true[c(2, 1)], guide = "none") +
  labs(y = "Import growth, %", x = "", title = "EU import growth from Russia neighbours after war")
```
...
ggplot(aes(x = reporter, y = egrowth, fill = epos), data = eu_serf3) +
  ...
  theme_bw() +
  scale_fill_manual(values = cols_true[c(2, 1)], guide = "none") +
  labs(y = "Export growth, %", x = "", title = "EU sanctioned export growth to Russia after war")
```
...
ggplot(aes(x = reporter, y = egrowth, fill = epos), data = eu_senb3) +
  ...
  theme_bw() +
  scale_fill_manual(values = cols_true[c(2, 1)], guide = "none") +
  labs(y = "Export growth (capped at 500%), %", x = "", title = "EU sanctioned export growth to Russia neighbours after war")
```
...
The full table with all the markets, their after war export value and growth from the same period in year 2021 can be found [here](https://github.com/vzemlys/ruimpeks/tree/main/xlsx/eu_sanctioned_export_growth.xlsx). Note the file size is 11MB.
